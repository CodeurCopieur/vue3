...
<template>
    ...
    <li  
      ...
      class="w-full px-4 py-4 border-b border-gray-200 rounded-t-lg dark:border-gray-600 flex flex-wrap flex-col">
      <header class="w-full flex justify-between mb-3 relative">
        <div>
          <button type="button" class="w-40 text-white bg-blue-700 hover:bg-blue-800 font-medium  text-sm px-5 py-2.5 mr-4 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">{{ task.whatWatch}}</button>
          <span>Total Time : {{task.time}}</span>
        </div>
        <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 h-6 top-0 text-sm px-1 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="defaultModal">
